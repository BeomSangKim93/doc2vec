...
# 유사도 체크하는 함수
# 단어의 유사도를 찾는 함수
def mostsimilar_word(model):
    print('검색 예) 포스터: \n', model.wv.most_similar('영화', topn=7), '\n')
    nsimilar = 7
    wsimilar = input("유사도가 궁금한 단어 ? = ")
    try:
        nsimilar = int(input("출력개수는 ? = "))
    except ValueError:
        print("1~300 사이의 숫자")
    ...
    try:
        print_words = model.wv.most_similar(wsimilar, topn=nsimilar)
    except :
        print("단어 정보가 없습니다.")
        mostsimilar_word(model)
    print(print_words)
    print('\n')

# 영화의 유사도를 찾는 함수
def mostsimilar_doc(model):
    print('검색 예) MOVIE_8628627: \n', model.docvecs.most_similar('MOVIE_8628627', topn=7), '\n')
    nsimilar = 7
    wsimilar = input("유사도가 궁금한 영화 ? = ")
    try:
        nsimilar = int(input("출력개수는 ? = "))
    except ValueError:
        print("1~300 사이의 숫자")
    ...
    # 문서정보 출력
    print('{0} 과 유사도가 높은 영화들 :\n'.format(wsimilar))
    try:
        print_docs = model.docvecs.most_similar(wsimilar, topn=nsimilar)
    except :
        print("영화 정보가 없습니다.")
        mostsimilar_doc(model)
    print(print_docs)
    print('\n')
...
def BSdoc2vectest():
    ...
    while 1:
        try:
            type_question = int(input('단어 유사도는 1, 영화 유사도는 2~98, 99는 끝남 ?'))
        except ValueError:
            print('숫자를 입력하세요')
            BSdoc2vectest()
        ...
        if type_question == 1:
            mostsimilar_word(model)      
        elif 1 < type_question and type_question < 99:
            mostsimilar_doc(model)
        if type_question == 99:
            return
